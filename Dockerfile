FROM debian:jessie
MAINTAINER John Fanjoy <jfanjoy@inetu.net>

ENV DESMAN_CONTAINERIZER docker

# persistent / runtime deps
RUN apt-get update && apt-get install -y ca-certificates curl cmake gawk libmysqlclient-dev \
	libxml2-dev libmcrypt-dev libicu-dev openssl build-essential binutils-dev \
	libcap-dev zlib1g-dev libtbb-dev libonig-dev libpcre3-dev \
	autoconf libtool libcurl4-openssl-dev wget memcached \
	libreadline-dev libncurses5-dev libmemcached-dev libbz2-dev \
	libc-client2007e-dev php5-mcrypt php5-imagick libgoogle-perftools-dev \
	libcloog-ppl-dev libelf-dev libdwarf-dev libunwind8-dev subversion \
	libtbb2 g++-4.8 gcc-4.8 libjemalloc-dev \
	libc6-dev libmpfr4 libgcc1 binutils \
	libc6 libc-dev-bin libc-bin libgomp1 \
	libstdc++-4.8-dev libstdc++6 \
	libarchive13 cmake-data libacl1 libattr1 \
	g++ cpp gcc make libboost-thread1.55.0 \
	libboost-thread-dev libgd2-xpm-dev \
	pkg-config libboost-system1.55-dev libboost-context1.55-dev \
	libboost-program-options1.55-dev libboost-filesystem1.55-dev libboost-regex1.55-dev \
	libmagickwand-dev libiberty-dev libevent-dev libxslt-dev libgoogle-glog-dev \
	automake libldap2-dev libkrb5-dev libyaml-dev gperf ocaml-native-compilers 
	libwww-curl-perl lsof \
	--no-install-recommends \
	&& rm -rf /var/lib/apt/lists/*

# hphpize / hhvm-ext-install deps
RUN apt-get update && apt-get install -y git-core autoconf gcc libc-dev make pkg-config --no-install-recommends && rm -r /var/lib/apt/lists/*

RUN install -dT -o www-data -m2755 /var/www/ && \
	install -dT -o www-data -m2755 /build

VOLUME ["/var/desman/data"]

ENV PHP_INI_DIR /etc/hhvm/
RUN mkdir $PHP_INI_DIR

ENV HHVM_VERSION HHVM-3.7.1

RUN set -x \
	&& git clone https://github.com/facebook/hhvm.git /tmp/hhvm \
	&& cd /tmp/hhvm \
	&& git checkout tags/${HHVM_VERSION} \
	&& git submodule update --init --recursive \
	&& cmake . -DMYSQL_UNIX_SOCK_ADDR=/dev/null \
	&& make -j"$(nproc)" \
	&& make install \
	&& make clean \
	&& rm -rf $(pwd) \
	&& cd -

RUN /usr/bin/update-alternatives --install /usr/bin/php php /usr/local/bin/hhvm 60

# once we work everything out, this needs to go above with the rest of the packages
RUN apt-get update -q && apt-get install -y nginx libwww-curl-perl rsync

EXPOSE 80

# remove this for prod
RUN apt-get install -y -qq vim

WORKDIR /build
ADD . /build/

# ADD .openshift/action_hooks/deploy /build/deploy
# ADD config/hhvm/php.ini $PHP_INI_DIR/

RUN apt-get install sudo
RUN mv -v /build/.openshift/action_hooks/deploy /usr/bin/ && \
    mv -v /build/.inetu/bin/hhvm-ext-* /usr/local/bin/ && \
    install -m644 -o www-data -g www-data /build/config/hhvm.hdf /etc/hhvm/hhvm.hdf && \
    install -m644 -o www-data -g www-data /build/config/nginx.conf /etc/nginx/sites-available/default && \
    install -m2755 -o www-data -g www-data -dT /var/run/hhvm && \
    install -m2755 -o www-data -g www-data -dT /var/log/hhvm 

ENV DESMAN_CONTAINERIZER docker
# USER www-data
# CMD ["hhvm", "--mode", "server"]
CMD ["/bin/bash"]
