...
const Filter = () => {
    ...
    const searchFilteredPlaylists = useMemo(() => {
        ...
        return playlists.filter((playlist) => {
            ...
            if (playlist.mainCategory?.some(cat => cat.toLowerCase().includes(query))) return true;
            
            // Buscar en los géneros (array)
            if (playlist.genre?.some(gen => gen.toLowerCase().includes(query))) return true;
            
            // Buscar en los moods (array)
            if (playlist.moods?.some(mood => mood.toLowerCase().includes(query))) return true;
