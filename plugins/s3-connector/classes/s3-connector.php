...
class S3_Connector extends AWS_Plugin_Base {
	...
	function admin_menu() {
    	...
    	if ( isset( $submenu[$this->plugin_slug][0][0] ) ) {
    		$submenu[$this->plugin_slug][0][0] = __( 'Settings', 's3-connector' );
		}
	...
	function handle_post_request() {
		...
		if ( empty( $_POST['_wpnonce'] ) || !wp_verify_nonce( $_POST['_wpnonce'], 'aws-save-settings' ) ) {
			die( __( "Cheatin' eh?", 's3-connector' ) );
		}
	...
	function get_client() {
		if ( !$this->get_access_key_id() || !$this->get_secret_access_key() ) {
			return new WP_Error( 'access_keys_missing', sprintf( __( 'You must first <a href="%s">set your AWS access keys</a> to use this addon.', 's3-connector' ), 'admin.php?page=' . $this->plugin_slug ) );
		}
